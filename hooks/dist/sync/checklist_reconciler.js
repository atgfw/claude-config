...
import { computeChecklistHash, mergeChecklists, diffChecklists, } from './checklist_utils.js';
import { parseGitHubChecklist, parseOpenSpecTasks, parsePlanChecklist, parseClaudeTasks, } from './checklist_parsers.js';
/**
 ...
 */
export function parseByType(artifactType, content) {
    switch (artifactType) {
        case 'github_issue':
            return parseGitHubChecklist(content);
        case 'openspec':
            return parseOpenSpecTasks(content);
        case 'plan':
            return parsePlanChecklist(content);
        case 'claude_task':
            return parseClaudeTasks(content);
    }
}
/**
 ...
 */
export function detectDrift(entry, artifactType, content) {
 ...
 */
export function findOrCreateEntry(registry, artifactType, artifactId) {
 ...
 */
export function updateSyncSource(entry, artifactType, artifactId, contentHash) {
    const now = new Date().toISOString();
 ...
 */
export function reconcileArtifact(artifactType, artifactId, content) {
    ...
    const diff = diffChecklists(entry.checklist_items, artifactItems);
    // Merge items
 ...
 */
export function getRegistryState(artifactType, artifactId) {
    const registry = loadRegistry();
    let entry;
    if (artifactType === 'github_issue') {
        const issueNum = Number.parseInt(artifactId, 10);
 ...
 */
export function linkArtifacts(primaryType, primaryId, links) {
    const registry = loadRegistry();
 ...
 */
export function getLinkedArtifacts(artifactType, artifactId) {
    const registry = loadRegistry();
    const entry = findOrCreateEntry(registry, artifactType, artifactId);
    return {
 ...
 */
export function checkAllDrift(artifactType, artifactId, currentContents) {
    const registry = loadRegistry();
    const entry = findOrCreateEntry(registry, artifactType, artifactId);
    const drifted = [];
    for (const [type, content] of Object.entries(currentContents)) {
...
//# sourceMappingURL=checklist_reconciler.js.map