 ...
 */
function generateCriterionId(text) {
 ...
 */
export function parseSuccessCriteria(projectRoot) {
    const criteria = [];
 ...
 */
export function loadCriteriaStatus(projectRoot) {
 ...
 */
export function saveCriteriaStatus(status) {
 ...
 */
export function updateCriteriaStatus(projectRoot) {
    ...
    const status = {
        ...
        verifiedCount,
        percentComplete: criteria.length > 0 ? Math.round((verifiedCount / criteria.length) * 100) : 100,
    };
 ...
 */
export function verifyCriterion(projectRoot, criterionIdOrText, verifiedBy, evidence) {
    const status = updateCriteriaStatus(projectRoot);
    const criterion = status.criteria.find((c) => c.id === criterionIdOrText || c.text.toLowerCase().includes(criterionIdOrText.toLowerCase()));
    if (!criterion) {
        return false;
    }
    criterion.verified = true;
 ...
 */
export function getUnverifiedCriteria(projectRoot) {
    const status = updateCriteriaStatus(projectRoot);
 ...
 */
export function areAllCriteriaVerified(projectRoot) {
    const status = updateCriteriaStatus(projectRoot);
 ...
 */
export function formatCriteriaStatus(status) {
    const lines = [];
    lines.push(`Success Criteria: ${status.verifiedCount}/${status.totalCount} (${status.percentComplete}%)`);
    lines.push('');
...
//# sourceMappingURL=success_criteria_tracker.js.map