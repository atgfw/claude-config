...
import { loadRegistry as loadEscalationRegistry, saveRegistry as saveEscalationRegistry, generateSymptomHash, findBySymptomHash, updateStatus as updateEscalationStatus, linkToCorrection, } from './escalation_registry.js';
 ...
 */
export function recordCorrection(symptom, rootCause, hookToPrevent) {
    ...
    try {
        ...
        log(`[LEDGER-WARN] Could not link to escalations: ${error.message}`);
    }
    return entry;
}
/**
 ...
 */
function crossReferenceEscalations(correctionId, symptom) {
 ...
 */
export function markHookImplemented(correctionId, hookName) {
    const ledger = loadLedger();
    ...
    if (entry) {
        ...
        try {
            ...
            log(`[LEDGER-WARN] Could not update escalations: ${error.message}`);
        }
    }
}
/**
 ...
 */
function updateLinkedEscalations(symptom, hookName) {
    const symptomHash = generateSymptomHash(symptom);
    const registry = loadEscalationRegistry();
    const matchingEscalations = findBySymptomHash(registry, symptomHash);
    if (matchingEscalations.length > 0) {
 ...
 */
export function recordRecurrence(correctionId) {
    const ledger = loadLedger();
    const entry = ledger.entries.find((e) => e.id === correctionId);
    if (entry) {
 ...
 */
export function findSimilarCorrections(symptom) {
    const ledger = loadLedger();
 ...
 */
export function getUnresolvedCorrections() {
    const ledger = loadLedger();
 ...
 */
export function getRecurringCorrections() {
    const ledger = loadLedger();
...
//# sourceMappingURL=correction_ledger.js.map