 ...
 */
export function saveToEnv(service, key) {
    ...
    // Find existing entry
    const existingIndex = lines.findIndex((l) => l.startsWith(`${service.envVar}=`) || l.startsWith(`export ${service.envVar}=`));
 ...
 */
export function saveToMcpConfig(service, key) {
    const mcpPaths = [
 ...
 */
export function saveKey(service, key) {
 ...
 */
export function keyExistsInEnv(envVar) {
    const envPath = path.join(getClaudeDir(), '.env');
    if (!fs.existsSync(envPath))
        return false;
 ...
 */
export function getKeyFromEnv(envVar) {
    const envPath = path.join(getClaudeDir(), '.env');
    if (!fs.existsSync(envPath))
        return undefined;
    const content = fs.readFileSync(envPath, 'utf-8');
