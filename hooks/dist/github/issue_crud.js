...
export function createIssue(opts) {
    ...
    // Duplicate detection
    try {
        const searchResult = execSync(`gh issue list --search "${title.replace(/"/g, '\\"')}" --state open --json number,title`, { encoding: 'utf-8', timeout: 15000 }).trim();
    ...
    try {
        ...
        const output = execSync(`gh issue create --title "${title.replace(/"/g, '\\"')}" --body "${body.replace(/"/g, '\\"')}"${labelArg}`, { encoding: 'utf-8', timeout: 15000 }).trim();
 ...
 */
export function closeIssue(issueNumber, _reason) {
    try {
 ...
 */
export function createFromCorrection(entry) {
 ...
 */
export function createFromEscalation(entry) {
 ...
 */
export function createFromOpenSpec(changeId, summary) {
...
//# sourceMappingURL=issue_crud.js.map