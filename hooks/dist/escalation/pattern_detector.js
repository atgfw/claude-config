 ...
 */
import { loadRegistry, findBySymptomHash, calculatePriority, } from '../ledger/escalation_registry.js';
// ============================================================================
 ...
 */
export function detectPatterns(registry) {
 ...
 */
export function shouldTriggerProposal(symptomHash, registry) {
    const reg = registry ?? loadRegistry();
 ...
 */
export function getPatternsNeedingProposals(registry) {
 ...
 */
export function calculatePatternPriority(entry, config) {
 ...
 */
export function groupByCategory(registry) {
    const reg = registry ?? loadRegistry();
 ...
 */
export function groupBySeverity(registry) {
    const reg = registry ?? loadRegistry();
    const groups = new Map();
    for (const entry of Object.values(reg.escalations)) {
    ...
    return Array.from(groups.values()).sort((a, b) => severityOrder.indexOf(a.category) - severityOrder.indexOf(b.category));
}
/**
 * Group similar escalations by symptom similarity
 * Uses word overlap for fuzzy matching
 */
export function groupBySimilarity(threshold = 0.5, registry) {
    const reg = registry ?? loadRegistry();
 ...
 */
export function calculateSymptomSimilarity(symptom1, symptom2) {
    const words1 = new Set(symptom1
        .toLowerCase()
        .replace(/[^\w\s]/g, '')
        .split(/\s+/)
        .filter((w) => w.length > 2));
    const words2 = new Set(symptom2
        .toLowerCase()
        .replace(/[^\w\s]/g, '')
        .split(/\s+/)
        .filter((w) => w.length > 2));
 ...
 */
export function getPatternSummary(registry) {
    const patterns = detectPatterns(registry);
...
//# sourceMappingURL=pattern_detector.js.map