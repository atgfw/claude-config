 ...
 */
export function parseWorkflowRegistry(projectRoot) {
 ...
 */
export function findLocalWorkflows(projectRoot) {
    const workflows = new Map(); // name -> path
 ...
 */
export function findStaleReferences(projectRoot, validIds) {
    const issues = [];
    ...
    for (const dir of searchDirs) {
        if (fs.existsSync(dir)) {
            ...
            filesToScan.push(...mdFiles.filter((f) => f.includes('AUDIT') || f.includes('design.md') || f.includes('README')));
        }
    }
 ...
 */
export function detectDrift(projectRoot, cloudWorkflows // id -> name
) {
    const issues = [];
    // Load local registry
 ...
 */
export function saveDriftReport(report) {
...
//# sourceMappingURL=documentation_drift_detector.js.map