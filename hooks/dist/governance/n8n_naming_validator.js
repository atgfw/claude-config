 ...
 */
export function containsBracketTag(name) {
 ...
 */
export function containsVersionNumber(name) {
 ...
 */
export function containsArbitraryInteger(name) {
 ...
 */
export function suggestSystemPrefix(tag) {
...
export function validateWorkflowName(name) {
    ...
    if (bracketCheck.found && bracketCheck.tag !== '[DEV]') {
        ...
        if (suggestion) {
            ...
            result.errors.push(`Bracket tag "${bracketCheck.tag}" not allowed in n8n (use native tags instead)`);
        }
    }
    ...
    if (integerCheck.found) {
        result.warnings.push(`Integer "${integerCheck.integer}" in name - consider if this is necessary`);
    }
    return result;
}
/**
...
export function validateNodeName(name) {
    ...
    if (integerCheck.found) {
        result.valid = false;
        result.errors.push(`Integer "${integerCheck.integer}" not allowed in node names (unless canonical)`);
 ...
 */
export async function n8nNamingValidatorHook(input) {
    const toolName = input.tool_name ?? '';
    const toolInput = input.tool_input;
    log(`[NAMING] Validating n8n operation: ${toolName}`);
    // Extract workflow payload
    const workflowName = toolInput['name'] ?? '';
    ...
    if (!validation.valid) {
        ...
        const suggestionSummary = validation.suggestions.length > 0
            ...
            : '';
        logBlocked(`Naming validation failed: ${errorSummary}`, 'n8n Naming Conventions - see hooks/docs/n8n-governance.md');
        return {
            hookSpecificOutput: {
...
//# sourceMappingURL=n8n_naming_validator.js.map