...
export async function specCompletenessValidatorHook(input) {
    ...
    if (!specFound) {
        ...
        return {
            hookSpecificOutput: {
                hookEventName: 'PreToolUse',
                permissionDecision: 'allow',
                permissionDecisionReason: `GOVERNANCE WARNING: No spec file found for "${path.basename(filePath)}". ` +
    ...
    if (!specValid) {
        logBlocked(`Spec incomplete for ${path.basename(filePath)}`, `Spec errors: ${specErrors.join(', ')}`);
        return {
            hookSpecificOutput: {
                hookEventName: 'PreToolUse',
                permissionDecision: 'deny',
                permissionDecisionReason: `BLOCKED: Spec incomplete for "${path.basename(filePath)}". ` +
...
//# sourceMappingURL=spec_completeness_validator.js.map