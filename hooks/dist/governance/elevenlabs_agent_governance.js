 ...
 */
function findSimilarAgents(name, prompt, liveAgents) {
    const matches = [];
 ...
 */
async function handlePreCreateAgent(input) {
    ...
    if (top && top.similarity >= 70) {
        ...
        logBlocked(`Found very similar agent "${top.name}" (${top.similarity}% match) in ElevenLabs cloud`, 'Before creating agents, check existing agents via LIVE API to prevent duplication.');
        return {
            hookSpecificOutput: {
                hookEventName: 'PreToolUse',
 ...
 */
export async function elevenlabsAgentGovernanceHook(input) {
...
//# sourceMappingURL=elevenlabs_agent_governance.js.map