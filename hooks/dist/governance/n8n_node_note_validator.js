 ...
 */
export function isPlaceholderNote(note) {
 ...
 */
export function isDuplicateOfName(note, nodeName) {
    ...
    if (normalizedNote.length < normalizedName.length + 10 &&
 ...
 */
export function containsActionVerb(note) {
    const commonVerbs = [
        'fetch',
        'get',
        'set',
 ...
 */
export function validateNodeNote(node) {
    const result = {
        valid: true,
        errors: [],
        warnings: [],
 ...
 */
export function validateWorkflowNotes(payload) {
    const result = {
        valid: true,
        errors: [],
        warnings: [],
 ...
 */
export async function n8nNodeNoteValidatorHook(input) {
    ...
    if (!validation.valid) {
        ...
        logBlocked(`Node documentation validation failed: ${errorSummary}`, 'n8n Node Documentation - see hooks/docs/n8n-governance.md');
        return {
            hookSpecificOutput: {
...
//# sourceMappingURL=n8n_node_note_validator.js.map