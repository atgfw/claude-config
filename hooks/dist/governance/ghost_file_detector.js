 ...
 */
function isGhostFile(filePath) {
    const filename = path.basename(filePath).toLowerCase();
    return GHOST_FILE_PATTERNS.some((pattern) => filename.includes(pattern));
}
export async function ghostFileDetectorHook(input) {
    ...
    if (isGhostFile(filePath)) {
        ...
        return {
            hookSpecificOutput: {
                hookEventName: 'PreToolUse',
                permissionDecision: 'allow',
                permissionDecisionReason: `GOVERNANCE WARNING: Reading "${filename}" which may be a ghost file (stale/outdated). ` +
...
//# sourceMappingURL=ghost_file_detector.js.map