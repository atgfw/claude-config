 ...
 */
async function fetchAllWorkflowsFromN8n() {
 ...
 */
function findSimilarWorkflows(name, liveWorkflows) {
    const matches = [];
 ...
 */
async function handlePreCreate(input) {
    ...
    if (top && top.similarity >= 70) {
        ...
        logBlocked(`Found very similar workflow "${top.name}" (${top.similarity}% match)`, 'Before creating workflows, check for existing similar workflows to prevent duplication.');
        return {
            hookSpecificOutput: {
                hookEventName: 'PreToolUse',
 ...
 */
async function handlePreDelete(input) {
    const toolInput = input.tool_input;
 ...
 */
export async function n8nWorkflowGovernanceHook(input) {
...
//# sourceMappingURL=n8n_workflow_governance.js.map