 ...
 */
function checkConsoleStatements(code) {
    const errors = [];
    const lines = code.split('\n');
    for (const [i, line] of lines.entries()) {
 ...
 */
function checkDebuggerStatements(code) {
    const errors = [];
    const lines = code.split('\n');
    for (const [i, line] of lines.entries()) {
 ...
 */
export function lintCodeNodeContent(code) {
    const result = {
        valid: true,
        errors: [],
        warnings: [],
 ...
 */
export function isComplexInlineExpression(expression) {
 ...
 */
export function extractCodeFromNode(node) {
 ...
 */
export function validateWorkflowCodeNodes(payload) {
    const result = {
        valid: true,
        errors: [],
        warnings: [],
        ...
        if (node.parameters) {
            for (const [key, value] of Object.entries(node.parameters)) {
                if (typeof value === 'string' && isComplexInlineExpression(value)) {
                    result.warnings.push(`Node "${node.name}": Complex expression in "${key}" - consider moving to code node for testability`);
                }
            }
        }
    }
 ...
 */
export async function codeNodeLintingGateHook(input) {
...
//# sourceMappingURL=code_node_linting_gate.js.map