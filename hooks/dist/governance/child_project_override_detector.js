 ...
 */
function isWithinClaudeDir(filePath) {
 ...
 */
function isBannedFile(filePath) {
 ...
 */
function isBannedDirectory(filePath) {
    const parts = filePath.split(path.sep);
    return parts.some((part) => BANNED_CHILD_DIRECTORIES.includes(part));
}
export async function childProjectOverrideDetectorHook(input) {
    ...
    if (isBannedFile(filePath)) {
        const filename = path.basename(filePath);
        logBlocked(`Attempted to create ${filename} in child project`, 'Child projects MUST NOT override global configuration');
        return {
            hookSpecificOutput: {
                hookEventName: 'PreToolUse',
                permissionDecision: 'deny',
                permissionDecisionReason: `BLOCKED: Child projects MUST NOT override global configuration. ` +
    ...
    if (isBannedDirectory(filePath)) {
        logBlocked(`Attempted to create hooks/ directory in child project`, 'Child projects MUST NOT have local hooks');
        return {
            hookSpecificOutput: {
                hookEventName: 'PreToolUse',
                permissionDecision: 'deny',
                permissionDecisionReason: `BLOCKED: Child projects MUST NOT create local hooks. ` +
...
//# sourceMappingURL=child_project_override_detector.js.map