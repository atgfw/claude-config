...
function validateBranchName(branchName) {
    ...
    if (protectedBranches.has(branchName.toLowerCase())) {
        result.valid = false;
        result.errors.push(`"${branchName}" is a protected branch and cannot be created directly`, 'Create a feature branch instead: feature/<description>');
        return result;
    }
    ...
    if (!match?.groups) {
        ...
        if (branchName.includes('/')) {
            ...
            if (prefix && !allowedPrefixes.includes(prefix.toLowerCase())) {
                result.errors.push(`Invalid prefix "${prefix}"`, `Allowed prefixes: ${allowedPrefixes.join(', ')}`);
            }
        }
        else {
 ...
 */
function isNewBranchCommand(command) {
    return (/git\s+checkout\s+-b\b/.test(command) ||
 ...
 */
export async function branchNamingValidatorHook(input) {
    // Extract command
    ...
    if (!validation.valid) {
        logWarning('Branch name does not follow naming conventions', 'All branches should follow the pattern: prefix/kebab-case-description');
        log('');
        log('ERRORS:');
...
//# sourceMappingURL=branch_naming_validator.js.map