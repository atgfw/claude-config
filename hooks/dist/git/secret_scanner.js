...
const secretPatterns = [
    ...
    {
        name: 'AWS Secret Key',
        pattern: /(?:aws_secret_access_key|secret_key|aws_secret)\s*[=:]\s*["']?([a-z\d/+=]{40})["']?/gi,
        severity: 'CRITICAL',
    },
    // GitHub
    {
...
function formatMatches(matches) {
    ...
    for (const [file, fileMatches] of byFile) {
        ...
        for (const match of fileMatches) {
            lines.push(`  Line ${match.line}: ${match.pattern.name} [${match.pattern.severity}]`, `    > ${match.preview}`);
        }
    }
    return lines.join('\n');
}
 ...
 */
export async function secretScannerHook(input) {
    // Extract command
    ...
    logBlocked(`${matches.length} secret(s) detected in staged files`, 'NEVER commit secrets to version control.');
    log('');
...
//# sourceMappingURL=secret_scanner.js.map