 ...
 */
function isImplementationCode(filePath) {
 ...
 */
function hasProjectDirective(projectRoot) {
 ...
 */
function findPendingAudits(projectRoot) {
    const pending = [];
 ...
 */
function findIncompleteSpecs(projectRoot) {
    const incomplete = [];
    // Look in openspec directory
    const openspecDir = path.join(projectRoot, 'openspec');
    if (!fs.existsSync(openspecDir)) {
    ...
    const incompletePatterns = [
        { regex: /\bTBD\b/i, marker: 'TBD' },
        { regex: /\bTODO\b/i, marker: 'TODO' },
        { regex: /\bPENDING\b/i, marker: 'PENDING' },
        { regex: /\bINCOMPLETE\b/i, marker: 'INCOMPLETE' },
    ];
            ...
            for (let i = 0; i < lines.length; i++) {
                ...
                for (const { regex, marker } of incompletePatterns) {
                    if (regex.test(line)) {
                        ...
                        if (line.includes('| Status |') || line.includes('marker'))
                            continue;
                        incomplete.push({
                            file: specFile,
                            line: i + 1,
 ...
 */
function findUnverifiedCriteria(projectRoot) {
    const unverified = [];
    const directivePath = path.join(projectRoot, 'PROJECT-DIRECTIVE.md');
 ...
 */
function performGateCheck(filePath) {
    const result = {
        passed: true,
        projectRoot: null,
    ...
    if (result.incompleteSpecs.length > 0) {
        result.errors.push(`Found ${result.incompleteSpecs.length} incomplete spec marker(s) (TBD/TODO/PENDING)`);
        result.passed = false;
    }
 ...
 */
export async function preBuildGateHook(input) {
    ...
    logBlocked('BUILD phase blocked - governance pre-requisites not met', 'Before ANY spec is approved for blueprinting, a 3rd-party enforcer audit is required.');
    const reasonParts = [];
...
//# sourceMappingURL=pre_build_gate.js.map