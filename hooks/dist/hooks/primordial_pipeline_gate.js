...
import { loadRegistry, checkEntityHealth, getEntityByPath, formatHealthReport, formatEntityProgress, } from '../ledger/test_run_registry.js';
// ============================================================================
// Configuration
// ============================================================================
/**
...
// Formatting
// ============================================================================
function generateBlockMessage(result) {
    const lines = [];
    lines.push('');
    ...
    if (result.healthResult) {
        ...
        lines.push(formatHealthReport(result.healthResult)
            .split('\n')
            .map((l) => `  ${l}`)
            .join('\n'));
        lines.push('');
...
// Hook Implementation
// ============================================================================
export async function primordialPipelineGateHook(input) {
    ...
    if (result.healthResult) {
        ...
        const entity = getEntityByPath(registry, filePath);
        if (entity) {
            log(`[PRIMORDIAL GATE] ${formatEntityProgress(entity, registry.config.requiredNovelRuns)} ${entity.status.toUpperCase()}`);
        }
    }
...
//# sourceMappingURL=primordial_pipeline_gate.js.map