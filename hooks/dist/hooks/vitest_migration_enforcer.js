 ...
 */
function findJestUsage(content) {
    const matches = [];
 ...
 */
export async function vitestMigrationEnforcerHook(input) {
    ...
    if (filePath && /jest\.config\.(js|ts|mjs|cjs)$/i.test(filePath)) {
        logBlocked('Jest config file detected', 'Vitest is the ONLY approved test framework. When test requests arise: If project has Jest: Migrate to Vitest first.');
        log('');
        log('MIGRATION STEPS:');
        ...
        return {
            hookSpecificOutput: {
                ...
                permissionDecisionReason: 'Jest configuration file detected. ' +
    ...
    if (jestMatches.length > 0) {
        logBlocked(`Jest usage detected: ${jestMatches.join(', ')}`, 'Vitest is the ONLY approved test framework. When test requests arise: If project has Jest: Migrate to Vitest first.');
        log('');
        log('MIGRATION STEPS:');
        log('1. Uninstall Jest:');
        log('   npm uninstall jest @types/jest ts-jest jest-environment-jsdom');
        ...
        return {
            hookSpecificOutput: {
                hookEventName: 'PreToolUse',
                permissionDecision: 'deny',
                permissionDecisionReason: `Jest usage detected: ${jestMatches.join(', ')}. ` +
...
//# sourceMappingURL=vitest_migration_enforcer.js.map