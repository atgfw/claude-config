 ...
 */
function findOpenspecChangePath(filePath) {
 ...
 */
function findIncompleteMarkers(filePath) {
    const markers = [];
 ...
 */
function scanForIncompleteMarkers(openspecPath) {
    const allMarkers = [];
 ...
 */
function checkEnforcerAudit(openspecPath) {
 ...
 */
function countResearchSources(openspecPath) {
 ...
 */
function checkNodeSpecs(openspecPath) {
    const required = [];
    const found = [];
 ...
 */
function checkDirectiveAlignment(projectRoot, openspecPath) {
...
function auditPlan(filePath) {
    ...
    if (result.researchSourcesFound < result.researchSourcesRequired) {
        result.violations.push(`Research sources: ${result.researchSourcesFound}/${result.researchSourcesRequired} minimum`);
    }
    // Check for incomplete markers
    result.incompleteMarkers = scanForIncompleteMarkers(openspecPath);
    if (result.incompleteMarkers.length > 0) {
        result.violations.push(`${result.incompleteMarkers.length} incomplete marker(s) (TBD/TODO/PENDING) found`);
    }
 ...
 */
export async function planCompletenessGateHook(input) {
    ...
    log(`  [${audit.researchSourcesFound >= audit.researchSourcesRequired ? 'x' : ' '}] Research sources: ${audit.researchSourcesFound}/${audit.researchSourcesRequired}`);
    ...
    log('');
    if (!audit.isComplete) {
        logBlocked(`Plan incomplete - ${audit.violations.length} issue(s)`, 'Plans must meet all governance requirements before approval');
        log('');
        log('VIOLATIONS:');
...
//# sourceMappingURL=plan_completeness_gate.js.map