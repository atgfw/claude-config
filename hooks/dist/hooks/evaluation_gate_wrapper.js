...
import { evaluationGateHook, defaultEvaluationGateConfig, } from './evaluation_gate.js';
/**
...
async function fetchWorkflowExecutions(workflowId) {
    try {
        const response = await fetch(`${N8N_CONFIG.baseUrl}/api/v1/executions?workflowId=${workflowId}&limit=100`, {
            headers: {
                'X-N8N-API-KEY': N8N_CONFIG.apiKey,
                Accept: 'application/json',
            },
 ...
 */
export async function evaluationGateWrapperHook(input) {
    ...
    const isRemovingDevTag = operations.some((op) => op.type === 'removeTag' && op.tagId === TAG_IDS.dev);
    if (!isRemovingDevTag) {
...
//# sourceMappingURL=evaluation_gate_wrapper.js.map