 ...
 */
function parseWorkflowResult(toolOutput) {
    if (!toolOutput)
        return null;
    ...
    if (typeof toolOutput === 'object') {
        return extractWorkflowData(toolOutput);
    }
    return null;
}
/**
 ...
 */
export async function n8nWorkflowActivationHook(input) {
    ...
    if (!workflowResult || !workflowResult.id) {
        ...
        return {
            hookSpecificOutput: {
                hookEventName: 'PostToolUse',
                additionalContext: 'WARNING: Could not parse workflow ID from result. Verify workflow is active in n8n UI for webhook access.',
            },
        };
    }
...
//# sourceMappingURL=n8n_workflow_activation.js.map