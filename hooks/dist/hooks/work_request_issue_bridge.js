 ...
 */
function detectWorkRequest(prompt) {
 ...
 */
function deriveSystemPrefix(workingDir) {
    const dirName = path.basename(workingDir).toLowerCase();
    if (dirName.includes('n8n'))
        return 'n8n';
    if (dirName.includes('claude') || dirName === '.claude')
        return 'infra';
    if (dirName.includes('elevenlabs'))
        return 'elevenlabs';
    if (dirName.includes('servicetitan'))
        return 'servicetitan';
    if (dirName.includes('rewst'))
        return 'rewst';
    return 'general';
}
/**
 ...
 */
function isDuplicateOfOpenIssue(title) {
    try {
        const registry = loadRegistry();
        for (const entry of registry.entries) {
            if (entry.status !== 'open' || !entry.goal_summary)
                continue;
...
// Hook Implementation
// ============================================================================
async function workRequestIssueBridge(input) {
...
//# sourceMappingURL=work_request_issue_bridge.js.map