 ...
 */
async function taskGoalSync(input) {
    ...
    if (tool_name === 'TaskCreate') {
        return handleTaskCreate(tool_input, output);
    }
    return handleTaskUpdate(sessionId, tool_input, output);
}
/**
 * Handle TaskCreate - note that a task was created.
 * The task becomes the goal focus when marked in_progress.
 */
function handleTaskCreate(input, output) {
 ...
 */
function handleTaskUpdate(sessionId, input, output) {
 ...
 */
export function getCurrentTaskGoal(sessionId) {
 ...
 */
export function isTaskCurrentFocus(taskId, sessionId) {
    const resolvedSessionId = sessionId ?? getSessionId();
    const stack = loadGoalStack(resolvedSessionId);
    const current = stack.stack[0];
