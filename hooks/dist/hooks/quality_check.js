...
class QualityChecker {
    ...
    warnings = [];
    autofixes = [];
    constructor(filePath, projectRoot, config) {
    ...
    async checkTypeScript() {
        ...
        const result = await this.runCommand('npx', ['tsc', '--noEmit', '--project', configPath], this.projectRoot);
        if (result.code !== 0) {
    ...
    async checkXO() {
        ...
        if (result.code !== 0) {
            ...
            if (result.stderr.includes('NoFilesFoundError') ||
            ...
            if (this.config.xo.autofix) {
                ...
                if (recheck.code === 0) {
                    ...
                    if (recheck.stderr.includes('NoFilesFoundError') ||
    ...
    async checkPrettier() {
        ...
        const checkResult = await this.runCommand('npx', ['prettier', '--check', this.filePath], this.projectRoot);
...
function isFileModifyingTool(toolName) {
    // Direct match
...
export async function qualityCheckHook(input) {
    ...
    return {
        hookSpecificOutput: {
            hookEventName: 'PostToolUse',
            additionalContext: result.warnings.length > 0
...
//# sourceMappingURL=quality_check.js.map