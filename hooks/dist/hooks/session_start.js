...
import { getSessionId, setSessionIdEnv, ensureSessionDir, pushGoal, createIssueGoal, } from '../session/goal_stack.js';
 ...
 */
async function initializeSessionGoals(sessionId, workingDirectory, _issues, successes) {
 ...
 */
function detectGitHubIssueFromContext(_workingDirectory) {
    try {
...
async function buildKanbanStep(_issues, successes) {
    ...
    try {
        ...
        try {
            ...
            const needsProposal = registry.entries.filter((e) => e.status === 'open' && !e.openspec_change_id && e.github_issue);
