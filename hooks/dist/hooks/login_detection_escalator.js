 ...
 */
export async function loginDetectionEscalatorHook(input) {
    ...
    logBlocked('Login/authentication page detected', 'Escalate login prompts to the user. Do not attempt to bypass authentication.');
    log('');
 ...
 */
function extractAnalyzableContent(input) {
 ...
 */
function detectLoginIndicators(content) {
    const indicators = [];
 ...
 */
function detectUrlLoginIndicators(input) {
    const url = extractUrl(input);
    if (!url) {
        return [];
    }
 ...
 */
function buildEscalationMessage(url, _contentIndicators, _urlIndicators) {
    const parts = [];
...
//# sourceMappingURL=login_detection_escalator.js.map