 ...
 */
export function isNpmCliAvailable() {
    try {
 ...
 */
export async function searchNpmPackages(query, limit = 10) {
    ...
    try {
        ...
        const rawPackages = JSON.parse(output);
        const packages = rawPackages.slice(0, limit).map((p) => ({
            name: p.name,
            version: p.version,
 ...
 */
export async function searchAutomationPackages(domain) {
...
export function formatSearchResults(result) {
    ...
    const lines = [`npm search results for: ${result.query}`, ''];
    for (const pkg of result.packages) {
        lines.push(`### ${pkg.name}`, `- **Version:** ${pkg.version}`, `- **npm URL:** ${pkg.links.npm}`);
...
//# sourceMappingURL=npm_searcher.js.map