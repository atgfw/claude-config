  ...
  if (description?.trim()) {
    ...
    if (pastTenseIndicators.includes(firstWord)) {
      result.warnings.push(
        `Use imperative mood: "${firstWord}" -> "${firstWord.replace(/ed$/, '')}" or "${firstWord.replace(/d$/, '')}"`
      );
    }
  } else {
    result.valid = false;
 ...
 */
export async function commitMessageValidatorHook(
  ...
  if (!validation.valid) {
    logWarning(
      ...
      'All commits should follow Conventional Commits 1.0.0 standard'
    );
