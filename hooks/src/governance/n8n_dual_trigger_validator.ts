 ...
 */
function isExecuteWorkflowTrigger(node: N8nNode): boolean {
 ...
 */
function isWebhookTrigger(node: N8nNode): boolean {
 ...
 */
function isMergeNode(node: N8nNode): boolean {
 ...
 */
function getWebhookPath(node: N8nNode): string | undefined {
  if (!isWebhookTrigger(node)) return undefined;
  return node.parameters?.['path'] as string | undefined;
}
 ...
 */
function validateDualTrigger(workflow: N8nWorkflow): ValidationResult {
  ...
  if (!result.hasWebhookTrigger) {
    result.valid = false;
    result.errors.push(
      'Subworkflow has executeWorkflowTrigger but no webhook trigger for API access'
    );
 ...
 */
function extractWorkflowData(input: PreToolUseInput): N8nWorkflow | null {
  const toolInput = input.tool_input;
  if (!toolInput || typeof toolInput !== 'object') return null;

  // Check for workflow data in various possible locations
  const workflow = toolInput['workflow'] as N8nWorkflow | undefined;
  ...
  if (toolInput['nodes'] && Array.isArray(toolInput['nodes'])) {
    return {
      ...
      connections: toolInput['connections'] as Record<string, unknown> | undefined,
    };
  }
  ...
  if (workflowJson) {
    try {
      return JSON.parse(workflowJson) as N8nWorkflow;
    } catch {
      return null;
    }
  }
 ...
 */
function generateDualTriggerTemplate(workflowName: string): string {
 ...
 */
export async function n8nDualTriggerValidatorHook(
        ...
        return {
          hookSpecificOutput: {
            ...
            permissionDecisionReason:
              'Google Sheets nodes banned. Use n8n Data Tables (n8n-nodes-base.n8nTables) instead.',
          },
        };
      }
    }
  ...
  logBlocked(
    'Subworkflow missing dual trigger pattern',
    'n8n subworkflows MUST have both executeWorkflowTrigger AND webhook trigger for API access'
  );
