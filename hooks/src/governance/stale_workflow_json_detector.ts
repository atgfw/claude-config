 ...
 */
function isWorkflowJson(data: unknown): boolean {
...
export async function staleWorkflowJsonDetectorHook(
  input: PreToolUseInput
): Promise<PreToolUseOutput> {
  if (!FILE_TOOLS.has(input.tool_name)) return allow();
  ...
  if (!filePath.toLowerCase().endsWith('.json')) return allow();

  // Skip known config filenames
  const filename = path.basename(filePath).toLowerCase();
  if (EXCLUDED_FILENAMES.has(filename)) return allow();
  if (filename.endsWith('.config.json')) return allow();

  // Skip excluded directories
  if (isInExcludedDir(filePath)) return allow();
  ...
  if (input.tool_name === 'Write' && typeof toolInput['content'] === 'string') {
    try {
      isWorkflow = isWorkflowJson(JSON.parse(toolInput['content'] as string));
    } catch {
      // Not valid JSON, skip
    }
  }
