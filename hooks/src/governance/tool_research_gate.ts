 ...
 */
export function isWrapperPath(filePath: string): boolean {
 ...
 */
export function getResearchDocumentPath(wrapperFilePath: string): string {
 ...
 */
export function researchDocumentExists(wrapperFilePath: string): boolean {
 ...
 */
export function validateResearchDocument(content: string): ResearchValidationResult {
  ...
  for (const sectionMatch of toolSections) {
    ...
    const decision = (decisionMatch[1]?.toUpperCase() ?? 'REJECTED') as 'ACCEPTED' | 'REJECTED';
    ...
    if (decision === 'REJECTED' && stars && stars >= highStarThreshold) {
      result.warnings.push(
        `Rejected tool "${name}" with ${stars.toLocaleString()} stars - ensure rationale is documented`
      );
    }
  }
  ...
  if (result.toolsEvaluated.length === 0) {
    result.valid = false;
    result.errors.push(
      'No tools evaluated - research document must evaluate at least one existing tool'
    );
  }
  ...
  if (decisionMatch) {
    result.finalDecision = decisionMatch[1]?.toUpperCase() as 'BUILD' | 'USE';
  } else {
    result.valid = false;
 ...
 * Load the registry
 */
export function loadRegistry(): ToolResearchRegistry {
  ...
  try {
    ...
    return JSON.parse(content) as ToolResearchRegistry;
  } catch {
    return {
      version: '1.0.0',
      lastUpdated: new Date().toISOString(),
 ...
 * Save the registry
 */
export function saveRegistry(registry: ToolResearchRegistry): void {
  const registryPath = getRegistryPath();
 ...
 * Generate entry ID
 */
function generateEntryId(problemDomain: string, projectPath: string): string {
 ...
 */
export async function toolResearchGateHook(input: PreToolUseInput): Promise<PreToolUseOutput> {
  ...
  if (!fs.existsSync(researchPath)) {
    logBlocked(
      `No research document found at ${researchPath}`,
      'Tool Selection Protocol - research required before creating wrappers'
    );
  ...
  logAllowed(
    `Research validated - ${validation.finalDecision} decision with ${validation.toolsEvaluated.length} tools evaluated`
  );
