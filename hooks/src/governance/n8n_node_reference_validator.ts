...
function extractNodeReferences(code: string): string[] {
  const refs: string[] = [];
  // Match $('...') and $("...")
  const pattern = /\$\(\s*['"]([^'"]+)['"]\s*\)/g;
...
function validateReferences(nodes: N8nNode[]): { valid: boolean; errors: string[] } {
...
async function handler(input: PreToolUseInput): Promise<PreToolUseOutput> {
  ...
  if (tool_name === 'mcp__n8n-mcp__n8n_create_workflow') {
    nodes = (tool_input.nodes as N8nNode[]) || [];
    ...
    const operations =
      (tool_input.operations as Array<{
        type: string;
        node?: N8nNode;
        updates?: { parameters?: { jsCode?: string } };
        nodeId?: string;
        name?: string;
      }>) || [];
    ...
    for (const op of operations) {
      if (op.type === 'updateNode' && op.updates?.parameters?.jsCode) {
        ...
        if (refs.length > 0) {
          log(
            `[!] Node update contains $() references: ${refs.join(', ')}. ` +
      ...
      if (op.type === 'addNode' && op.node?.parameters?.jsCode) {
        ...
        if (refs.length > 0) {
          log(
            `[!] New node "${op.node.name}" contains $() references: ${refs.join(', ')}. ` +
    ...
    return {
      hookSpecificOutput: {
        ...
        permissionDecisionReason:
          `Stale node references detected. ${result.errors.join('; ')}. ` +
