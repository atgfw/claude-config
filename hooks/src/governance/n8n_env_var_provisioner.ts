...
const ENV_VAR_PATTERNS: Record<string, EnvVarConfig> = {
  // 3CX variables
  '3CX_BASE_URL': {
    name: '3CX_BASE_URL',
 ...
 */
function extractEnvVarReferences(workflow: N8nWorkflow): Set<string> {
 ...
 */
function getRequiredEnvVars(envVarNames: Set<string>): EnvVarConfig[] {
 ...
 */
function getClaudeEnvPath(): string {
 ...
 */
function readExistingEnvVars(): Map<string, string> {
 ...
 */
function appendEnvVars(configs: EnvVarConfig[]): number {
  const envPath = getClaudeEnvPath();
  const existing = readExistingEnvVars();
  const toAdd: EnvVarConfig[] = [];
  ...
  if (toAdd.length > 0) {
    const lines: string[] = ['', '# Auto-provisioned by n8n-env-var-provisioner hook'];
 ...
 */
function usesVarsPattern(workflow: N8nWorkflow): boolean {
  const jsonString = JSON.stringify(workflow);
 ...
 */
export async function n8nEnvVarProvisionerHook(input: PreToolUseInput): Promise<PreToolUseOutput> {
  ...
  if (usesVarsPattern(workflow)) {
    logBlocked(
      'Workflow uses $vars pattern which requires n8n Variables license',
      'Use $env.VAR_NAME pattern instead (reads from server environment)'
    );
    return {
      hookSpecificOutput: {
        hookEventName: 'PreToolUse',
        ...
        permissionDecisionReason:
          'BLOCKED: Workflow uses $vars["..."] pattern which requires n8n Variables license. ' +
