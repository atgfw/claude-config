 ...
 */
export function isKebabCase(path: string): boolean {
 ...
 */
export function containsTestWord(path: string): boolean {
 ...
 */
export function hasAuthenticationHeader(parameters: Record<string, unknown>): boolean {
  const auth = parameters['authentication'];
  if (auth !== 'headerAuth') return false;

  const options = parameters['options'] as Record<string, unknown> | undefined;
  if (!options) return false;

  const headerAuth = options['headerAuth'] as Record<string, unknown> | undefined;
  ...
  return (
    typeof name === 'string' && name.length > 0 && typeof value === 'string' && value.length > 0
  );
}
 ...
 */
export function pathMatchesWorkflowName(path: string, workflowName: string): boolean {
  ...
  // Exact match
  if (path === kebabWorkflowName) return true;

  // Path contains the workflow name (longer descriptive paths allowed)
  if (path.includes(kebabWorkflowName)) return true;

  // Workflow name contains the path (path is a shorter version)
  if (kebabWorkflowName.includes(path)) return true;
 ...
 */
export function generateSecretKeyName(webhookPath: string): string {
 ...
 */
export function validateWebhookPath(path: string, workflowName: string): WebhookValidationResult {
  ...
  if (containsTestWord(path)) {
    result.valid = false;
    result.errors.push(
      `Webhook path "${path}" should not contain the word "test" (n8n has built-in test triggers)`
    );
 ...
 */
export function validateWebhookNode(node: N8nNode, workflowName: string): WebhookValidationResult {
  const result: WebhookValidationResult = {
    valid: true,
    errors: [],
    warnings: [],
  ...
  if (!node.webhookId || node.webhookId.trim() === '') {
    ...
    result.suggestions.push(
      'Add webhookId: "unique-id" to the webhook node (use UUID or descriptive slug)'
    );
  }
 ...
 */
export function validateWebhookPayload(payload: N8nWorkflowPayload): WebhookValidationResult {
  const result: WebhookValidationResult = {
    valid: true,
    errors: [],
    warnings: [],
 ...
 */
function extractWorkflowData(input: PreToolUseInput): N8nWorkflowPayload | null {
  const toolInput = input.tool_input;
  if (!toolInput || typeof toolInput !== 'object') return null;

  // Check for workflow data in various possible locations
  const workflow = toolInput['workflow'] as N8nWorkflowPayload | undefined;
  ...
  const workflowJson = toolInput['workflow_json'] as string | undefined;
  if (workflowJson) {
    try {
      return JSON.parse(workflowJson) as N8nWorkflowPayload;
    } catch {
      return null;
    }
  }
 ...
 */
export async function n8nWebhookPathValidatorHook(
  ...
  if (!validation.valid) {
    ...
    logBlocked(
      `Webhook path validation failed: ${errorSummary}`,
      'Webhook Path Naming Convention - see hooks/docs/n8n-governance.md'
    );
