...
import {
  type ScraplingAction,
  type ScraplingActionResult,
  validateActions,
} from './scrapling_actions.js';
 ...
 */
export async function runScrapling(config: ScraplingRunnerConfig): Promise<ScraplingRunnerResult> {
  ...
  return new Promise((resolve, reject) => {
    ...
    child.on('close', (code) => {
      ...
      if (code !== 0 && stderr && !stdout) {
        try {
          const errorData = JSON.parse(stderr) as { error?: string };
      ...
      try {
        resolve(JSON.parse(stdout) as ScraplingRunnerResult);
      } catch {
...
export async function quickFetch(
  url: string,
  actions: ScraplingAction[] = [{ action: 'screenshot', filename: 'page.png' }]
): Promise<ScraplingRunnerResult> {
  return runScrapling({
    url,
    actions,
 ...
 */
export async function explorePage(url: string): Promise<ScraplingRunnerResult> {
  return runScrapling({
    url,
    actions: [
...
export async function fillForm(
  ...
  const actions: ScraplingAction[] = [{ action: 'screenshot', filename: 'form-before.png' }];
  ...
  if (submitSelector) {
    actions.push(
      ...
      { action: 'screenshot', filename: 'form-submitted.png' }
    );
  }
...
`);
  ...
  const actions = actionsArgument ? (JSON.parse(actionsArgument) as ScraplingAction[]) : undefined;
