...
export type ScraplingActionType =
  | 'screenshot'
  | 'click'
  | 'fill'
  | 'type'
 ...
 */
export const SCRAPLING_ACTIONS: Record<
  ScraplingActionType,
  {
    description: string;
 ...
 */
export function validateAction(action: ScraplingAction): { valid: boolean; errors: string[] } {
  const errors: string[] = [];
  ...
  for (const parameter of def.requiredParams) {
    if (!(parameter in action) || action[parameter as keyof ScraplingAction] === undefined) {
 ...
 */
export function validateActions(actions: ScraplingAction[]): {
  valid: boolean;
  errors: Array<{ step: number; errors: string[] }>;
} {
  const allErrors: Array<{ step: number; errors: string[] }> = [];
 ...
 */
export function generatePythonPageAction(
  actions: ScraplingAction[],
  functionName = 'page_action'
): string {
  const lines: string[] = [`def ${functionName}(page):`, `    results = []`];
  ...
  for (const [index, action] of actions.entries()) {
    ...
    lines.push(
      `    try:`,
      `        result_${index} = ${code}`,
      `        results.append({'step': ${index}, 'action': '${action.action}', 'status': 'ok', 'value': str(result_${index})[:500] if result_${index} else None})`,
      `    except Exception as e:`,
      `        results.append({'step': ${index}, 'action': '${action.action}', 'status': 'error', 'error': str(e)})`
    );
  }
 ...
 */
export function generateScraplingScript(
  options: ScraplingFetchOptions,
  actions: ScraplingAction[]
): string {
