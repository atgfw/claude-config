 ...
 */
export function escalate(params: EscalateParams): EscalationResult {
 ...
 */
export function escalateWithProject(
  ...
  if (existingInProject) {
    ...
    isNovel = false;
    log(
      `[ESCALATION] Incremented occurrence (${entry.occurrenceCount}): ${params.symptom.substring(0, 50)}...`
    );
    ...
    isNovel = false;
    log(
      `[ESCALATION] Cross-project pattern detected (${entry.crossProjectCount} projects): ${params.symptom.substring(0, 50)}...`
    );
  } else {
    // New escalation
 ...
 * @returns Result containing the escalation entry and metadata
 */
export function escalateFromHook(
  hookName: string,
  params: Omit<EscalateParams, 'relatedHooks'> & { category?: EscalationCategory }
): EscalationResult {
  return escalate({
    ...params,
 ...
 */
export function escalateGovernance(
  symptom: string,
  context: string,
 ...
 */
export function escalateTesting(
  symptom: string,
  context: string,
  proposedSolution: string,
  severity: EscalationSeverity = 'medium'
 ...
 */
export function escalateTooling(
  symptom: string,
  context: string,
  proposedSolution: string,
  severity: EscalationSeverity = 'medium'
 ...
 */
export function escalateSecurity(
  symptom: string,
  context: string,
  proposedSolution: string,
