 ...
 */
export function loadRegistry(): EscalationRegistry {
  const registryPath = getRegistryPath();

  if (fs.existsSync(registryPath)) {
    try {
      const content = fs.readFileSync(registryPath, 'utf-8');
      const parsed = JSON.parse(content) as EscalationRegistry;
 ...
 */
export function saveRegistry(registry: EscalationRegistry): void {
  const registryPath = getRegistryPath();
 ...
 */
export function createEmptyRegistry(): EscalationRegistry {
  return {
    escalations: {},
    symptomIndex: {},
    projectIndex: {},
 ...
 */
export function generateSymptomHash(symptom: string): string {
...
export function createEscalation(
  ...
  const cooldownUntil = new Date(
    Date.now() + registry.config.cooldownMinutes * 60 * 1000
  ).toISOString();
 ...
 */
export function findBySymptomHash(
  registry: EscalationRegistry,
  symptomHash: string
): EscalationEntry[] {
  ...
  return ids
    ...
    .filter((e): e is EscalationEntry => e !== undefined);
}

/**
 * Find escalations by symptom hash and project path
 */
export function findBySymptomHashAndProject(
  registry: EscalationRegistry,
  symptomHash: string,
  projectPath: string
 ...
 */
export function getById(registry: EscalationRegistry, id: string): EscalationEntry | undefined {
 ...
 */
export function getByProject(registry: EscalationRegistry, projectPath: string): EscalationEntry[] {
 ...
 */
export function incrementOccurrence(
  registry: EscalationRegistry,
  id: string
): EscalationEntry | undefined {
 ...
 */
export function addRelatedProject(
  registry: EscalationRegistry,
  id: string,
  projectPath: string
): EscalationEntry | undefined {
...
export function updateStatus(
  ...
  if (metadata) {
    if (metadata.resolutionNote) entry.resolutionNote = metadata.resolutionNote;
    if (metadata.rejectionReason) entry.rejectionReason = metadata.rejectionReason;
    if (metadata.generatedProposalPath)
      entry.generatedProposalPath = metadata.generatedProposalPath;
    if (metadata.implementedHookName) entry.implementedHookName = metadata.implementedHookName;
  }
 ...
 */
export function linkToCorrection(
  registry: EscalationRegistry,
  escalationId: string,
  correctionId: string
): void {
 ...
 */
export function getPendingEscalations(registry: EscalationRegistry): EscalationEntry[] {
 ...
 */
export function getPatternDetectedEscalations(registry: EscalationRegistry): EscalationEntry[] {
 ...
 */
export function getHighPriorityEscalations(registry: EscalationRegistry): EscalationEntry[] {
  return Object.values(registry.escalations).filter(
    (e) =>
 ...
 */
export function getActionableEscalations(registry: EscalationRegistry): EscalationEntry[] {
  return Object.values(registry.escalations).filter(
    (e) => e.status === 'pending' || e.status === 'pattern-detected'
  );
}
 ...
 */
export function getUnresolvedEscalations(registry: EscalationRegistry): EscalationEntry[] {
  return Object.values(registry.escalations).filter(
    (e) => e.status !== 'resolved' && e.status !== 'rejected' && e.status !== 'hook-implemented'
  );
}
 ...
 */
export function checkPatternThreshold(registry: EscalationRegistry, symptomHash: string): boolean {
  const entries = findBySymptomHash(registry, symptomHash);
 ...
 */
export function calculatePriority(entry: EscalationEntry, config: EscalationConfig): number {
 ...
 */
export function getByPriority(registry: EscalationRegistry): EscalationEntry[] {
  const entries = getActionableEscalations(registry);
  return entries.sort(
    (a, b) => calculatePriority(b, registry.config) - calculatePriority(a, registry.config)
  );
}
 ...
 */
export function getStats(registry: EscalationRegistry): {
  total: number;
  pending: number;
  ...
  const byCategory: Record<EscalationCategory, number> = {
    governance: 0,
    testing: 0,
    tooling: 0,
    pattern: 0,
  ...
  return {
    ...
    highPriority: entries.filter(
      (e) => (e.severity === 'high' || e.severity === 'critical') && e.status === 'pending'
    ).length,
    byCategory,
    bySeverity,
  };
