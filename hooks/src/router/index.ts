...
import type { ToolRouterConfig, ToolPreference } from '../types.js';
 ...
 */
export function loadRouterConfig(): ToolRouterConfig {
  const configPath = getRouterConfigPath();

  if (fs.existsSync(configPath)) {
    try {
      const content = fs.readFileSync(configPath, 'utf-8');
      return JSON.parse(content) as ToolRouterConfig;
    } catch {
 ...
 */
export function saveRouterConfig(config: ToolRouterConfig): void {
  const configPath = getRouterConfigPath();
 ...
 */
function checkConditions(preference: ToolPreference): boolean {
 ...
 */
export function route(context: RoutingContext): RoutingDecision {
 ...
 */
export function getRoutingRecommendation(context: RoutingContext): string {
 ...
 */
export function suggestFileEditTool(filePath: string): RoutingDecision {
  return route({
    operation: 'file-edit',
    filePath,
  });
 ...
 */
export function suggestBrowserTool(
  operation: 'navigate' | 'screenshot' | 'interact',
  url?: string
): RoutingDecision {
