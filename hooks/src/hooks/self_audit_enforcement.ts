...
// Block counter functionality is unused but kept for future implementation
// const BLOCK_COUNTER_FLAG = '.self-audit-block-count';
// const MAX_CONSECUTIVE_BLOCKS = 3;

interface SelfAuditChecklist {
  directiveAligned: boolean; // Work serves PROJECT-DIRECTIVE.md
  specsComplete: boolean; // All node specs are complete
  enforcerAudited: boolean; // 3rd party audit requested/completed
  researchDone: boolean; // Research requirements met
  noIncompleteMarkers: boolean; // No TBD/TODO/PENDING markers
  testsPlanned: boolean; // Test plan exists
  hierarchyRespected: boolean; // Hierarchical development followed
}

/**
 * Find openspec changes in current directory
 */
function findOpenspecChanges(dir: string): string[] {
 ...
 */
function normalizeChecklist(data: Record<string, unknown>): SelfAuditChecklist | null {
  ...
  if ('self_audit_completed' in data && data.self_audit_completed === true) {
    ...
    if ('checklist' in data && typeof data.checklist === 'object' && data.checklist !== null) {
      const nested = data.checklist as Record<string, boolean>;
  ...
  if ('directiveAligned' in data || 'specsComplete' in data) {
    return data as unknown as SelfAuditChecklist;
  }
 ...
 */
function checkSelfAuditFlag(): SelfAuditChecklist | null {
 ...
 */
function requiresParentAudit(changes: string[]): boolean {
 ...
 */
function validateChecklist(checklist: SelfAuditChecklist): string[] {
 ...
 */
function isInfrastructureWork(cwd: string): boolean {
 ...
 */
export async function selfAuditEnforcementHook(_input: StopInput): Promise<StopOutput> {
  ...
  if (!checklist) {
    log(
      `Self-audit required: Create ~/.claude/self-audit-completed.json with { "self_audit_completed": true, "checklist": { directiveAligned, specsComplete, enforcerAudited, researchDone, noIncompleteMarkers, testsPlanned, hierarchyRespected } } - all fields must be true.`
    );
  ...
  if (violations.length > 0) {
    log(
      `Self-audit incomplete (${violations.length} items): ${violations.join(', ')}. Update self-audit-completed.json with all fields set to true.`
    );
  ...
  if (requiresParentAudit(changes)) {
    for (const changePath of changes) {
      ...
      if (!auditStatus.hasRequest) {
        ...
        log(
          `Parent audit requested for ${changeName} (${request.id}) - proceeding without approval.`
        );
