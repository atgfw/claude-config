 ...
 */
function needsLLMEnrichment(fields: GoalFields): boolean {
 ...
 */
function generateFieldExtractionPrompt(
  issueNumber: number,
  issueTitle: string,
  currentFields: GoalFields
): string {
...
async function goalAutoDerivationSessionStart(
  ...
  if (result.hydrated) {
    ...
    if (derived.goal && needsLLMEnrichment(derived.goal.fields)) {
      ...
      if (issueNumber) {
        const enrichmentPrompt = generateFieldExtractionPrompt(
          issueNumber,
