 ...
 */
export function generateSummaryFilename(cwd: string): string {
 ...
 */
export function getSummaryFilePath(cwd: string): string {
 ...
 */
export function generateSummaryInstructions(cwd: string, pct: number): string {
 ...
 * Main hook function
 */
async function contextSummaryTrigger(
  ...
  try {
    ...
    const alert: ContextAlert = JSON.parse(alertContent);
    ...
    log(
      'CONTEXT SUMMARY TRIGGER: Error reading alert file: ' +
