 ...
 */
function parseTestOutput(output: string): { passed: number; failed: number; total: number } | null {
 ...
 */
function generateSuspicionMessage(history: TestHistory, latestRun: TestRunRecord): string {
 ...
 */
async function metavitestSuspicion(input: PostToolUseInput): Promise<PostToolUseOutput> {
  ...
  if (passRate === 1) {
    ...
    logTerse(
      `[+] Test run: ${results.passed}/${results.total} passed (streak: ${history.consecutivePerfectRuns})`
    );
  } else {
