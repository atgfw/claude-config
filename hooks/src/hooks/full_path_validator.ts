 ...
 */
function expandToAbsolute(relativePath: string): string {
 ...
 */
function extractPaths(toolName: string, toolInput: Record<string, unknown>): string[] {
 ...
 */
async function fullPathValidator(input: PreToolUseInput): Promise<PreToolUseOutput> {
  ...
  // Check each path
  const relativePaths: Array<{ path: string; suggested: string }> = [];
...
export {
  fullPathValidator as fullPathValidatorHook,
  isAbsolutePath,
  expandToAbsolute,
  extractPaths,
};
