 ...
 */
const VITEST_COMBINED_PATTERN =
  /import\s*\{[^}]*(describe|it|expect)[^}]*\}\s*from\s*['"]vitest['"]/;
 ...
 */
function isCodeNodeTestFile(filePath: string): boolean {
  const normalized = filePath.replace(/\\/g, '/');
  return (
    CODE_NODE_TEST_PATTERN.test(normalized) &&
 ...
 */
function hasWrongExtension(filePath: string): boolean {
 ...
 */
function hasVitestImports(content: string): boolean {
 ...
 */
function hasConsoleLogTesting(content: string): boolean {
 ...
 */
function hasFixtureImports(content: string): boolean {
 ...
 */
function getFixturesDir(testFilePath: string): string {
 ...
 */
function checkFixturesDirectory(testFilePath: string): {
  exists: boolean;
  hasInputs: boolean;
  hasExpected: boolean;
  files: string[];
 ...
 */
function validateCodeNodeTest(filePath: string, content: string): ValidationResult {
  ...
  if (hasWrongExtension(filePath)) {
    ...
    result.suggestions.push(
      `Rename to: ${filePath.replace(/\.test\.js$/, '.test.ts').replace(/\.spec\.js$/, '.spec.ts')}`
    );
  }
  ...
  if (hasVitestImports(content) && !hasFixtureImports(content)) {
    result.warnings.push(
      'No fixture imports detected - test data should be in external JSON files'
    );
    result.suggestions.push(
      'Create fixtures/ directory with *-input.json and *-expected.json files'
    );
    result.suggestions.push(
      "Import fixtures: import validInput from './fixtures/valid-input.json';"
    );
  }
  ...
  if (filePath.endsWith('.test.ts') || filePath.endsWith('.spec.ts')) {
    ...
    if (!fixturesCheck.exists) {
      result.warnings.push(
        'No fixtures/ directory found - create one with input/expected JSON files'
      );
    } else {
 ...
 */
function extractInput(input: PreToolUseInput): { filePath: string | null; content: string | null } {
