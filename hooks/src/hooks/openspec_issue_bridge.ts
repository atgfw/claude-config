...
export function extractChangeId(filePath: string): string | null {
 ...
 */
async function openspecIssueBridge(input: PostToolUseInput): Promise<PostToolUseOutput> {
  ...
  try {
    ...
    log(`[openspec-bridge] Issue creation failed: ${(error as Error).message}`);
  }
