 ...
 */
export async function postToolUseHook(input: PostToolUseInput): Promise<PostToolUseOutput> {
  ...
  if (toolName && isPlaywrightTool(toolName)) {
    ...
    if (isScraplingAvailable()) {
      logBlocked(
        'Playwright used when Scrapling is available',
        'USE SCRAPLING MCP for all browser automation tasks. Scrapling is preferred over Playwright.'
      );
      log('');
      log('PREFERRED USAGE:');
      ...
      return {
        hookSpecificOutput: {
          ...
          additionalContext:
            'WARNING: Playwright used when Scrapling available - prefer Scrapling MCP',
        },
      };
    }
  }
  ...
  if (toolName === 'Bash') {
    ...
    if (command && containsDirectPlaywright(command)) {
      logBlocked(
        'Direct Python Playwright detected',
        'USE SCRAPLING MCP for all browser automation tasks.'
      );
      log('');
 ...
 */
function extractCommand(input: PostToolUseInput): string | undefined {
 ...
 */
function containsDirectPlaywright(command: string): boolean {
  const patterns = [
    /from playwright/i,
    /import playwright/i,
