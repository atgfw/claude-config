 ...
 */
function extractWorkflowDependencies(workflowJson: unknown): WorkflowDependency[] {
  ...
  const workflow = workflowJson as Record<string, unknown>;
 ...
 */
function parseWorkflowInput(input: PreToolUseInput): {
  workflowName?: string;
  ...
  return {
    workflowName: toolInput.name as string,
 ...
 */
function checkDependenciesHealth(dependencies: WorkflowDependency[]): {
  unhealthyDeps: Array<{ dep: WorkflowDependency; result: HealthCheckResult }>;
  untestedDeps: WorkflowDependency[];
} {
  const registry = loadRegistry();
  const unhealthyDeps: Array<{ dep: WorkflowDependency; result: HealthCheckResult }> = [];
  const untestedDeps: WorkflowDependency[] = [];

  for (const dep of dependencies) {
    const entityId =
      dep.type === 'subworkflow' ? generateEntityId('subworkflow', dep.path) : dep.id;
 ...
 */
function checkHierarchicalTestingCompliance(
  workflowJson: unknown,
  operation: string
): {
  compliant: boolean;
  ...
  for (const { dep, result } of unhealthyDeps) {
    if (result.status === 'failing') {
      ...
      violations.push(
        `Dependency "${dep.name}" needs ${missing} more novel test runs (${result.novelRunCount}/3)`
      );
    }
    ...
    if (!result.childrenHealthy) {
      violations.push(
        `Dependency "${dep.name}" has unhealthy children: ${result.unhealthyChildren.join(', ')}`
      );
    }
  }
 ...
 */
export async function hierarchicalTestingGateHook(
  ...
  if (!compliance.compliant) {
    logBlocked(
      `Hierarchical testing violated - ${compliance.violations.length} issues`,
