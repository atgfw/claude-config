 ...
 */
function extractTagOperation(input: PreToolUseInput): {
  workflowId?: string;
  ...
  const removingDevTag = operations.some((op: Record<string, unknown>) => {
  ...
  const addingArchiveTag = operations.some((op: Record<string, unknown>) => {
    if (typeof op !== 'object' || !op) return false;
    const type = op['type'];
    const tagId = op['tagId'];
  ...
  return {
    hookSpecificOutput: {
      ...
      permissionDecisionReason:
        'Evaluation gate checks required - use evaluation-gate-wrapper hook to validate',
    },
  };
}
