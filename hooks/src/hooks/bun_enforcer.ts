...
export function detectBlockedCommands(command: string): DetectionResult {
  ...
  const matches: Array<{ found: string; replacement: string }> = [];
...
async function bunEnforcerHook(input: PreToolUseInput): Promise<PreToolUseOutput> {
