...
interface EscalationPattern {
  name: string;
  test: (input: PostToolUseInput) => boolean;
  symptom: (input: PostToolUseInput) => string;
  context: (input: PostToolUseInput) => string;
...
const ESCALATION_PATTERNS: EscalationPattern[] = [
  {
    ...
    context: (input) =>
      `Tool: ${input.tool_name}, File: ${JSON.stringify(input.tool_input).substring(0, 100)}`,
  ...
  },
  {
    ...
    symptom: (input) => {
      const result = String(input.tool_output ?? '');
      if (result.includes('command not found')) return 'Bash command not found error';
      if (result.includes('No such file or directory'))
        return 'File or directory not found in Bash';
      if (result.includes('Permission denied')) return 'Permission denied in Bash execution';
      return 'Bash command execution error';
    },
    context: (input) => {
      const cmd = (input.tool_input as { command?: string })?.command ?? 'unknown';
 ...
 */
export async function escalationTriggerHook(input: PostToolUseInput): Promise<PostToolUseOutput> {
  ...
  for (const pattern of ESCALATION_PATTERNS) {
    try {
      if (pattern.test(input)) {
        ...
        log(
          `[ESCALATION] Pattern detected: ${pattern.name}${result.isNovel ? ' (new)' : ' (existing)'}`
        );
      ...
      log(`[ESCALATION-ERROR] ${pattern.name}: ${(error as Error).message}`);
    }
  }
