...
import type {
  ...
  MeasuredBySection,
  WhichSection,
} from '../schema/task_specification.js';
...
export function validateStructuralCompleteness(
  spec: Partial<TaskSpecification>
): ValidationIssue[] {
  const issues: ValidationIssue[] = [];

  // Required sections
  const requiredSections: Array<{ key: keyof TaskSpecification; name: string }> = [
...
export function validateFocus(focus?: FocusDeclaration): ValidationIssue[] {
  const issues: ValidationIssue[] = [];

  if (!focus) return issues;
...
export function validateWhat(what?: WhatSection): ValidationIssue[] {
  const issues: ValidationIssue[] = [];

  if (!what) return issues;
  ...
  if (!what.desired_state) {
    ...
    if (what.desired_state.includes('will ') || what.desired_state.includes('should ')) {
      issues.push({
        ...
        suggestion:
          'Describe as if observing completed state: "The system handles..." not "will handle"',
      });
    }
  }
...
export function validateWhich(which?: WhichSection): ValidationIssue[] {
  const issues: ValidationIssue[] = [];

  if (!which) return issues;
...
export function validateLest(lest?: LestSection): ValidationIssue[] {
  const issues: ValidationIssue[] = [];

  if (!lest) return issues;
...
export function validateWith(withSection?: WithSection): ValidationIssue[] {
  const issues: ValidationIssue[] = [];

  if (!withSection) return issues;
...
export function validateMeasuredBy(measuredBy?: MeasuredBySection): ValidationIssue[] {
  const issues: ValidationIssue[] = [];

  if (!measuredBy) return issues;
...
export function detectAntiPatterns(spec: Partial<TaskSpecification>): ValidationIssue[] {
  const issues: ValidationIssue[] = [];
...
export function validateTaskSpec(spec: Partial<TaskSpecification>): ValidationResult {
  const issues: ValidationIssue[] = [
...
export function validateMinimalSpec(spec: Partial<MinimalTaskSpec>): ValidationResult {
  const issues: ValidationIssue[] = [];
 ...
 */
async function taskSpecValidatorHook(input: PreToolUseInput): Promise<PreToolUseOutput> {
  ...
  const metadata = toolInput.metadata as Record<string, unknown> | undefined;
  const spec = metadata?.task_spec as Partial<TaskSpecification> | undefined;
