 ...
 */
export function detectFabricatedVersions(text: string): VersionDetectionResult {
 ...
 */
export function detectFilenameVersioning(filename: string): VersionDetectionResult {
  const result: VersionDetectionResult = {
    found: false,
    patterns: [],
    locations: [],
 ...
 */
function isCanonicalException(identifier: string): boolean {
 ...
 */
function hasExistingVersioning(targetDir: string): boolean {
  if (!targetDir) {
    return false;
  }
 ...
 */
function contentAlreadyHasVersioning(filePath: string): boolean {
  try {
 ...
 */
export async function versionFabricationDetectorHook(
  ...
  if (isWriteOp) {
    contentToCheck = (toolInput['content'] as string) ?? '';
    filePath = (toolInput['file_path'] as string) ?? '';
  } else if (isEditOp) {
    contentToCheck = (toolInput['new_string'] as string) ?? '';
    filePath = (toolInput['file_path'] as string) ?? '';
  } else if (isBashOp) {
    contentToCheck = (toolInput['command'] as string) ?? '';
  ...
  const message =
    `Fabricated versioning detected: ${patternList}. ` +
