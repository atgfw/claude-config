 ...
 */ function isAllowedBrowserPattern(command: string): boolean {
 ...
 */
function containsDirectBrowserAutomation(command: string): boolean {
 ...
 */
export async function preBashHook(input: PreToolUseInput): Promise<PreToolUseOutput> {
  ...
  if (command && containsDirectBrowserAutomation(command)) {
    logBlocked(
      'Direct browser automation detected',
      'NEVER use Playwright/Puppeteer/Selenium directly via Bash. Use MCP tools instead.'
    );
    log('');
    ...
    return {
      hookSpecificOutput: {
        ...
        permissionDecisionReason:
          'Direct browser automation banned - use Scrapling MCP or Playwright MCP instead',
      },
    };
  }
