 ...
 */
export function generateSlug(symptom: string): string {
  return symptom
    .toLowerCase()
    .replace(/[^\w\s-]/g, '')
    .replace(/\s+/g, '-')
 ...
 */
export function generateChangeId(symptom: string): string {
 ...
 */
export function generateProposalMd(escalations: EscalationEntry[]): string {
  ...
  const evidenceRows = escalations
    .map(
      (e) =>
        `| ${e.id.substring(0, 8)} | ${e.projectName} | ${e.severity} | ${e.occurrenceCount} | ${e.timestamp.split('T')[0]} |`
    )
    .join('\n');
 ...
 */
export function generateTasksMd(escalations: EscalationEntry[]): string {
  const primary = escalations[0];
  if (!primary) return '';
 ...
 */
export function generateSpecMd(escalations: EscalationEntry[]): string {
  const primary = escalations[0];
  if (!primary) return '';
 ...
 */
function suggestHookEvent(category: string): string {
  switch (category) {
    case 'governance':
    case 'tooling':
      return 'PreToolUse';
 ...
 */
export function getOpenSpecChangesDir(): string {
 ...
 */
export function scaffoldOpenSpecChange(changeId: string): string {
 ...
 */
export function writeProposalFiles(changePath: string, escalations: EscalationEntry[]): void {
...
export function generateProposal(
  ...
  try {
    ...
    log(`[PROPOSAL] Error generating proposal: ${(error as Error).message}`);
    return null;
  }
}
 ...
 */
export function generateAllPendingProposals(registry?: EscalationRegistry): GeneratedProposal[] {
  const reg = registry ?? loadRegistry();
