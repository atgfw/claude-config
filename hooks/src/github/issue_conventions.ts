...
const VALID_TYPES = ['feat', 'fix', 'chore', 'docs', 'refactor', 'perf', 'test'] as const;
...
export const TITLE_REGEX = new RegExp(
  `^\\[(${SYSTEMS_PATTERN})\\] (${TYPES_PATTERN})(\\([a-z][a-z0-9-]*\\))?: [a-z].+[^.]$`
);
 ...
 */
export function validateTitle(title: string): { valid: boolean; warnings: string[] } {
  ...
  if (!TITLE_REGEX.test(title)) {
    ...
    } else if (!VALID_SYSTEMS.includes(systemMatch[1] as (typeof VALID_SYSTEMS)[number])) {
      warnings.push(`Invalid system "${systemMatch[1]}". Valid: ${VALID_SYSTEMS.join(', ')}`);
    }

    const typeMatch = title.match(/^\[[^\]]*\]\s*(\w+)/);
    if (typeMatch && !VALID_TYPES.includes(typeMatch[1] as (typeof VALID_TYPES)[number])) {
 ...
 */
export function validateBody(body: string): { valid: boolean; warnings: string[] } {
  const warnings: string[] = [];
 ...
 */
export function findDuplicates(title: string): { number: number; title: string }[] {
  ...
  try {
    issues = JSON.parse(output) as Array<{ number: number; title: string; state: string }>;
  } catch {
    return [];
  }
 ...
 */
function extractTitleFromCommand(command: string): string | undefined {
 ...
 */
function extractBodyFromCommand(command: string): string | undefined {
 ...
 * Issue Convention Validator Hook
 */
export async function issueConventionValidatorHook(
