 ...
 */
export function scanForOverrides(projectDir: string): Violation[] {
  ...
  if (fs.existsSync(mcpJsonPath)) {
    ...
    try {
      const content = JSON.parse(fs.readFileSync(mcpJsonPath, 'utf-8')) as Record<string, unknown>;
 ...
 */
export function checkCurrentProject(): Violation[] {
 ...
 */
export function reportViolations(violations: Violation[], recordToLedger = false): string {
  ...
  if (recordToLedger && errors.length > 0) {
    for (const v of errors) {
      recordCorrection(
        `Child project override detected: ${v.description}`,
        'Local configuration created in child project instead of using global',
        'pre-task-start hook should detect and block child project overrides'
      );
    }
  }
 ...
 */
export function enforceNoOverrides(): boolean {
 ...
 */
export function cleanupViolations(projectDir: string): {
  cleaned: string[];
  failed: string[];
} {
  ...
  for (const v of violations) {
    ...
    try {
      ...
      log(
        `[FAILED] Could not move ${v.path}: ${error instanceof Error ? error.message : 'Unknown error'}`
      );
    }
  }
