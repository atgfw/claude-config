 ...
 */
export async function propagateToGitHub(
  issueNumber: number,
  items: ChecklistItem[]
): Promise<boolean> {
  try {
 ...
 */
export async function propagateToFile(
  filePath: string,
  items: ChecklistItem[],
  artifactType: 'openspec' | 'plan'
): Promise<boolean> {
  try {
    ...
    const newChecklist =
      artifactType === 'openspec' ? renderToOpenSpecTasks(items) : renderToPlanChecklist(items);
 ...
 */
export async function propagateToLinkedArtifacts(
