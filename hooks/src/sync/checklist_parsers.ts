 ...
 */
export function parseGitHubChecklist(body: string): ChecklistItem[] {
 ...
 */
export function parseOpenSpecTasks(content: string): ChecklistItem[] {
  const items: ChecklistItem[] = [];
 ...
 */
export function parsePlanChecklist(content: string): ChecklistItem[] {
  const items: ChecklistItem[] = [];
  const lines = content.split('\n');
 ...
 */
export function parseClaudeTasks(tasks: ClaudeTask[]): ChecklistItem[] {
 ...
 */
export function renderToGitHubChecklist(items: ChecklistItem[]): string {
  return items
    .map((item) => {
 ...
 */
export function renderToOpenSpecTasks(items: ChecklistItem[]): string {
  return items
    .map((item) => {
      const mark = item.status === 'completed' ? 'x' : item.status === 'in_progress' ? '-' : ' ';
      return `- [${mark}] ${item.text}`;
 ...
 */
export function renderToPlanChecklist(items: ChecklistItem[]): string {
  return items
    .map((item) => {
      const mark = item.status === 'completed' ? 'x' : item.status === 'in_progress' ? '-' : ' ';
      return `- [${mark}] ${item.text}`;
 ...
 */
export function extractChecklistSection(body: string): {
  before: string;
  checklist: string;
  after: string;
} {
 ...
 */
export function replaceChecklistSection(body: string, newChecklist: string): string {
