...
export interface FocusDeclaration {
  issue_number?: number;
  title: string; // Imperative, ≤200 chars, pattern: {ACTION_VERB} {TARGET} {CONTEXT} {PURPOSE}
...
export interface WhatSection {
  desired_state: string; // 1-2 paragraphs, present tense
  acceptance_criteria: AcceptanceCriterion[];
  out_of_scope: string[]; // Explicit exclusions (≥2 required)
...
export interface WhySection {
  purpose: string; // Business/user value, non-technical
  strategic_objective?: string;
  project_milestone?: string;
  epic_reference?: string;
  user_story?: string; // "As a {persona}, I want {capability} so that {benefit}"
...
export interface TargetObject {
  object_title: string; // Verbatim exact name
  object_type: TargetObjectType;
  full_path: string; // Complete unambiguous path
...
export interface NegativeAcceptanceCriterion {
  ...
  when: string;
  must_not: string;
  lest: string; // Consequence of violation
}
...
export interface ExternalDependency {
  ...
  rate_limits?: string;
  sla?: string;
  fallback: string; // Required - what to do if unavailable
  documentation?: string;
}

export type HumanDependencyType =
  | 'approval'
  | 'information'
  | 'review'
  | 'access_grant'
...
// ============================================================================
// Minimal Quick Reference (for token-constrained injection)
// ============================================================================
