...
import {
  ...
  type DriftReport,
  type DriftIssue,
} from '../governance/documentation_drift_detector.js';

/**
 * Perform local-only drift detection
 * Does not call cloud APIs - only checks local file consistency
 */
export function checkLocalDrift(projectDirectory?: string): DriftReport {
 ...
 */
export function checkDocumentationDrift(projectDirectory?: string): SessionCheckResult {
