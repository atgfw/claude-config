 ...
 */
const garbagePatterns: Array<{ pattern: RegExp; description: string }> = [
 ...
 */
const namingViolations: Array<{ pattern: RegExp; description: string }> = [
 ...
 */
function checkGarbageFile(filename: string): { isGarbage: boolean; description?: string } {
 ...
 */
function checkNamingViolations(filename: string): { hasViolation: boolean; description?: string } {
 ...
 */
function getFileCategory(filename: string): 'script' | 'markdown' | 'image' | 'other' {
 ...
 */
function suggestSafeFilename(filename: string): string {
 ...
 */
function scanRootDirectory(projectDirectory: string): HygieneIssue[] {
  ...
  try {
      ...
      if (movableMarkdown.length > 3) {
        issues.push({
          ...
          suggestion: validateSuggestion(
            `mkdir -p docs && mv ${movableMarkdown.slice(0, 3).join(' ')} ... docs/`
          ),
          severity: 'info',
        });
      }
    ...
    if (counts.image > thresholds.maxImagesAtRoot) {
      issues.push({
        ...
        suggestion: validateSuggestion(
          `mkdir -p old/screenshots && mv *.png *.jpg *.jpeg old/screenshots/`
        ),
        severity: 'warn',
      });
    }
...
function writeToLedger(projectDirectory: string, issues: HygieneIssue[]): void {
  ...
  try {
    ...
    if (fs.existsSync(ledgerPath)) {
      ...
      registry = JSON.parse(content) as typeof registry;
    }
 ...
 */
export function auditFolderHygiene(projectDirectory?: string): HygieneAuditResult {
